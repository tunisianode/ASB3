...
public class ParameterConverter
{
            ...
            Map<String, String> namedParameters)
    {
        ...
        String[] toppingNames = parseStringValues(
                namedParameters.get(PARAM_NAME_INGREDIENTS));
        String[] sizeNames = parseStringValues(
                namedParameters.get(PARAM_NAME_SIZES));
        int[] toppingPrices = parseIntValues(
                namedParameters.get(PARAM_NAME_INGREDIENTS_PRICES));
        int[] sizePrices = parseIntValues(
                namedParameters.get(PARAM_NAME_SIZE_PRICES));
        int numberOfDefaultToppings = parseIntValue(
                namedParameters.get(PARAM_NAME_DEFAULT_INGRIEDENTS_NUMBER));
 
        return new Configuration(sizeNames, sizePrices, toppingNames,
                toppingPrices, numberOfDefaultToppings);
    }
 
    private static void checkParameterExistence(
            Map<String, String> namedParameters)
    {
        String[] allParams ={PARAM_NAME_INGREDIENTS, PARAM_NAME_INGREDIENTS_PRICES,PARAM_NAME_SIZES, PARAM_NAME_SIZE_PRICES,PARAM_NAME_DEFAULT_INGRIEDENTS_NUMBER};
